...
{card && (<> <Box className = "center" flexDirection={"column"}><Avatar
  alt="busienes card img"
  src={cardData.imageUrl}
  sx={{ width: 300, height: 300 }}
/>
<Typography variant="h2" color="initial">{card.title}</Typography>
<Typography variant="h3" color="initial">{card.subtitle}</Typography>
<Typography variant="h4" color="initial">{card.bizNumber}</Typography>


<Box sx={{display:user && user.isAdmin ? "block" : "none"}}>
<input type="number" onChange={(e) => cardData.bizNumber= e.target.value} />
<Button variant="text" color="primary" onClick={() => handleAdminNumber(cardId,cardData)}>
  new number
</Button>
</Box>
</Box>

<Grid container minHeight={180} spacing={1} mb={5}  mt={5} alignItems="center">
